...
export class HeroeService {
  ...
  GetHeroeById(id:number):HeroeDto
  {
    let heroeEncontrado : HeroeDto=null;

    for(var i=0; i<this.Heroes.length;i++)
    {
      if(this.Heroes[i].Id==id)
      {
        heroeEncontrado=this.Heroes[i];
        break;
      }
    }
  ...
  BusquedaHeroes(busqueda:string):HeroeDto[]
  {    

    let heroesBusqueda:HeroeDto[] = [];

    if(busqueda && busqueda!="")
    {
      this.Heroes.filter((dato)=>
      {
        if(dato.nombre.toLowerCase().includes(busqueda.toLowerCase()))
        {
          heroesBusqueda.push(dato);
        }         
